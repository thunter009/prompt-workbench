...
export function FolderReorgModal({ open, onClose }: FolderReorgModalProps) {
  ...
  const suggestedItems = useMemo(() => [...folderGroups.values()].flat(), [folderGroups])
  ...
  const handleApply = async () => {
    ...
    // Use mutable folderGroups as source of truth for folder assignments
    for (const [folderName, groupItems] of folderGroups) {
      const snippetIds = groupItems
        .filter((i) => selected.has(i.snippet.id))
        .map((i) => i.snippet.id)
      if (snippetIds.length === 0) continue
